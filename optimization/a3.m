...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % backprop
  % dE_do = error for output
  dE_do = class_output - data.targets;

  % dE_dh = error propagated to hidden units
  dE_dh = (model.hid_to_class' * dE_do) .* (logistic(hid_input) .* (1 - logistic(hid_input)));

  % average weight gradient for hidden-to-output weights
  m = size(data.inputs, 2);
  ret.hid_to_class += (1 / m) .* (dE_do * hid_output');

  % average weight gradient for input-to-hidden weights
  ret.input_to_hid += (1 / m) .* (dE_dh * data.inputs');
